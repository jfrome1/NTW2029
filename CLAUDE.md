...
## Course Pages Are Generated From the Planner Map And Should Not Be Edited

Since 2026-08-12, the Freeplane planner map `C:\Dev\repos\AI-projects\ntw2029 course design\data\planner\2610-planner.mm` is the authoritative source for course content, including the content that is posted on the website. Every page under `C:\Dev\repos\NTW2029\src\content\docs\course-ntw2029` is generated by `C:\Dev\repos\AI-projects\freeplane\scripts\export_website.groovy` based on planner map nodes, with the exception of the YAML frontmatter. That frontmatter is primarily copied from the existing md pages, but the groovy script can add or remove YAML lines about the draft status of the page.

Make all changes to website content in the planner map, not the markdown pages in `C:\Dev\repos\NTW2029\src\content\docs\course-ntw2029`. All changes made to .md files in that folder will be overwritten by the groovy script, except for the YAML frontmatter.

Every markdown page in `C:\Dev\repos\NTW2029\src\content\docs\course-ntw2029` should have a corresponding node in the map. Treat a page without such a node as a defect to resolve with the user.

The rule covers page bodies only. Astro configuration, components, styles, and everything outside `C:\Dev\repos\NTW2029\src\content\docs\course-ntw2029` are authored in this repo as normal.
...
| astro.config.mjs, build config, astro-compress, sidebar missing, page not appearing, broken or missing image, dev server not showing a change, restarting pnpm dev, draft frontmatter, content entry error, responsive CSS not applying, Vercel deployment or build logs, ignored build step, rehype plugins, site URL, log in to the site, inspect the live site, view as a student | `{AI_PROJECTS}/ntw2029 course design/context/project-website-context.md`                       |
