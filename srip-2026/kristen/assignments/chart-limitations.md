## **Average duration of a visit to the pages from each page type** 
...
**View used:** \`durations\_2520/2510\` 

**Filters:** Chart data is filterable by course week, student and date range. 
...
**View used:** \`events\_with\_study\_sessions\_2520/2510\` 

- Note: the \`durations\_2520/2510\` view is not used, as it filters out sessions with only 1 event. This undercounts the number of \`pageview\` events.
...
**2\. Not directly comparable to duration charts**

The duration chart uses the view \`durations\_2520/2510\`, while the page visit chart uses \`events\_with\_study\_sessions\_2520/2510\`, making them not comparable. This is due to durations being counted per session, which requires single-event sessions to be filtered out; hence reducing the number of pageviews. The \`events\_with\_study\_sessions\_2520/2510\` view does not filter out pageviews, making it a more accurate count of raw pageviews. However, as a result, the two charts should not be directly compared, as they measure distinct populations of events. 

## **Total site visits across all pages in each page type** 

### Overview

This chart shows the number of times a study session had an event occur on a page from the page type, for each page type. 

**View used:** \`events\_with\_study\_sessions\_2520/2510\` 

- Note: More information on this view can be found in \`current-views.md\`. 
...
Study sessions are estimated by ordering events by timestamp (regardless of session) – whenever two events have a time difference of \>30min, they are split and a new study session begins. However, this is an arbitrary threshold, and was chosen as it is a reasonable amount of time for a student to be inactive. Changing this threshold will lead to fewer study sessions, as more events will be counted as part of the same session instead of triggering a new split. On the other hand, lowering the threshold will produce more, shorter sessions, as gaps that were previously tolerated within a single session now get split into separate ones.

**2\. Mouse movements not appearing in the events table**

As it was found that mouse movements can keep a session active without any events being triggered, the 30min gap may be a student reading a page closely, rather than them going idle. As such, splitting on a fixed time gap could misclassify genuine reading time as session boundaries, which will underestimate both session duration and the number of events attributed to a single study session.

- Note: The userIdle/userActive events should be able to circumvent this issue (see \`current-views.md\`)

## **Activity breakdown by page**

### Overview

This chart breaks student activity down by page. Each bar is a different page, and shows the number of events triggered on that page. 
...
This chart breaks student activity down by event type. Each bar is a different event type, and shows the number of times that event type was triggered. 
...
## **Per-student total duration spent on website**

### Overview

This chart shows the total duration spent on the website for each student. 

**Filters:** Chart data is filterable by course week and by date range. 

**Limitations:**

\*\*See Average duration of a visit to the pages from each page type – Limitations. 

## **Per-student total page views on website**

## Overview

This chart shows the total number of page views triggered on the website for each student. 

**Filters:** Chart data is filterable by course week and by date range. 

**Limitations:**

\*\*See Total page visits to the pages from each page type – Limitations 

## **Activity by days to due date (paper & exercise assignments)**

### Overview

This chart maps student activity for each day in the window between the due date of a paper/exercise and the closest previous due date. For instance, if Exercise 1 was due on 4 Jan & Exercise 2 due on 6 Jan, the chart for Exercise 2 would show the activity for each of the two days between 4 and 6 Jan. Student activity is defined as the number of events triggered on all exercise pages, resource pages and course information pages, within the exercise window. 
...
## **Total duration buckets across page types**

### Overview

This chart categorizes the duration students spent on the website into 9 buckets: \<1min, 1-2min, 2-3min, 3-4min, 4-5min, 6-7min, 7-8min, 8-9min & 9-10min. Each bucket shows the total duration students spent for each page type. 

**Filters:** Chart data is filterable by course week, page type and date range. 

**Limitations:**

**1\. Duration mainly coming from \<1min buckets**

The \<1min bucket has the highest durations for all page types, suggesting that most students spent \<1min on the website. This is unlikely, and should be investigated more.   
