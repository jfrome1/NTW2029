**Goal:** This document is based on the five data questions defined for the project. For each question’s scenarios, it records the events captured in Posthog, and potential workarounds for limitations listed for each scenario.
...
_Definition:_ For a single page visit, the time between a student opening the page and when they leave.
...
| $pageview | Student opens tab | 1 | 1 |
| x  | x number of events (read, $autocapture, internalLinkClick etc) | 1 | 1 |
| _No event triggered_ | Student switches tab | \- | \- |
| _No event triggered_ | Student returns to tab before session expires | \- | \- |
| $pageleave | Student closes tab | 1 | 1 |
...
| :---- | :---- | :---- | :---- |
| $pageview | Student opens tab | 1 | 1 |
| _No event triggered_ | Student switches off tab & session expires | \- | \- |
...
| _No event triggered_ | Student switches off tab  |
...
_Definition:_ A site visit will be defined as an instance where a student opened the website and engaged with it for a meaningful period of time. A single visit may span multiple pages within the site.  
...
- However, this method has a limitation, which significantly affects Scenario 4 (see _2.4.3. Reasoning_ for details).
...
Reasoning: The same as Scenario 2’s reasoning. If necessary, differentiation between Scenarios 2 and 3 can be done (see _2.2.3. Reasoning_).
...
_Definition:_ A \`pageview\` (page gets loaded) is counted as an individual visit.
...
| Event | Triggered by |
| :---- | :---- |
| _No events captured_ | Student opens page in background  |
...
Not much to add here that isn’t already covered by the [analytics-data-limitations.md](http://analytics-data-limitations.md) file.
...
Similar to Question 4, not much to add here that isn’t already covered by the [analytics-data-limitations.md](http://analytics-data-limitations.md) file.
...
For the schedule-check scenarios that run into coverage gaps (background-tab on the schedule, or reaching it via back button or bookmark), a possible workaround could be identifying the sessions triggered on the schedule page that **do not** begin with a $pageview event, then manually checking if those sessions reflected actual engagement (e.g. read event triggered).
