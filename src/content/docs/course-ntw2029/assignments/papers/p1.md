...
Your paper _should not_ challenge Pinker's argument, so _don't_ use [:these strategies](#x-challenge):
...
Sometimes, pointing out problems with a source advances the academic conversation, but not when the criticisms are generic. Avoid these non-constructive approaches:
